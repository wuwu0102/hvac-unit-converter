...
function renderMobileResultCards(headers, rows){
  if (isCompactMode()) {
    return `<table class='compact-result-table'><thead><tr>${headers.map((h)=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map((cols)=>`<tr>${headers.map((_,idx)=>`<td>${cols[idx] ?? ''}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
  }
