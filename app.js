...
function renderMobileResultCards(headers, rows){
  ...
  if (isCompactMode()) {
    return `<table class='compact-result-table'><thead><tr>${safeHeaders.map((h)=>`<th>${h ?? ''}</th>`).join('')}</tr></thead><tbody>${safeRows.map((cols)=>`<tr>${safeHeaders.map((_,idx)=>`<td>${cols?.[idx] ?? ''}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
  }
  return `<div class='mobile-result-list'>${safeRows.map((cols)=>`<div class='mobile-result-card'>${safeHeaders.map((h,idx)=>`<div class='mobile-result-row'><span class='mobile-result-label'>${h}</span><span>${cols?.[idx] ?? ''}</span></div>`).join('')}</div>`).join('')}</div>`;
}
